...
The system SHALL support `jsonl`, `jsonl-compact`, and row-oriented output modes for capture decoding so the same packet stream can be consumed as a full structured record, a normalized packet-row record, or a tshark-like packet list.
...
- **WHEN** the user selects the dedicated packet-list-oriented output mode
