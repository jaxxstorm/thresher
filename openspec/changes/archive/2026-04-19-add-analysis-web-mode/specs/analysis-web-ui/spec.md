...
The system SHALL support a web analysis mode that starts a localhost-only HTTP server for `thresher analyze` and exposes the active session through a browser-accessible web UI.

#### Scenario: Web mode starts a localhost server
...
#### Scenario: Web mode does not expose non-local interfaces
- **WHEN** the web analysis server starts
- **THEN** it listens only on localhost interfaces
- **AND** it does not require remote authentication for the initial local-only implementation
