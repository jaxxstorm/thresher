# stateful-flow-analysis Specification
...
- **AND** the decoded JSON object can expose promoted top-level relative sequence or acknowledgment fields without removing the nested values
...
- **AND** the decoded JSON object can expose stable top-level analyzer flag fields for those same conditions
...
#### Scenario: Zero-window event is explicit
- **WHEN** a TCP packet advertises a zero receive window
- **THEN** the decoded JSON object includes machine-readable analyzer output for `zero_window`
- **AND** the zero-window condition can be consumed without parsing the free-text packet summary
